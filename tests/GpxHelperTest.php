...
class GpxHelperTest extends TestCase
{
    ...
    {
        $gpxExpected = trim(
            file_get_contents($this->testDir.'/test.gpx') ?: ''
        );

        $gpxHelper = new GpxHelper();

        $gpx = $gpxHelper->getRouteTrackGpx(
    ...
    {
        $gpxExpected = trim(
            file_get_contents($this->testDir.'/test_waypoints.gpx') ?: ''
        );

        $gpxHelper = new GpxHelper();

        $gpx = $gpxHelper->getWaypointsGpx(
            new MaxfieldParser($this->testDir.'/12345')
        );
    ...
    {
        $gpxExpected = trim(
            file_get_contents($this->testDir.'/test_route.gpx') ?: ''
        );

        $gpxHelper = new GpxHelper();

        $gpx = $gpxHelper->getRouteGpx(
            new MaxfieldParser($this->testDir.'/12345')
        );
    ...
    {
        $gpxExpected = trim(
            file_get_contents($this->testDir.'/test_track.gpx') ?: ''
        );

        $gpxHelper = new GpxHelper();

        $gpx = $gpxHelper->getTrackGpx(
            new MaxfieldParser($this->testDir.'/12345')
        );
